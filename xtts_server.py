...
def generate(progress):
    ...
    for speaker in text['speech']:
        ...
        for sentence_id, sentence in enumerate(speaker['speech']):
            ...
            while not final_gen:
                print(f"generating\t{speaker['speaker']}\t{sentence_id+1}/{len(speaker['speech'])}\t{round(audio_scale, 2)}\t{round(sentence['duration'], 2)}", end="\t")
                tts.tts_to_file(
                    text=sentence['translated'],
                    file_path=generated_audio_path,
                    speaker_wav=speaker['speaker_spliced'],
                    language=progress['languages']['target'],
                    speed = audio_scale
                )
                audio_written_file = AudioFileClip(generated_audio_path)
                print(f"{round(audio_written_file.duration, 2)}")
                if audio_scale < 1.5 and audio_written_file.duration > (sentence['duration']+min(sentence['duration']*0.05, 1)):
                    audio_scale = min(1.5, audio_scale*(audio_written_file.duration/sentence['duration']))
                    if audio_scale == 1.5:
                        break
                    continue
                else:
...
class JSONRequestHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        with open(data['progress'], mode='r') as progress_file:
